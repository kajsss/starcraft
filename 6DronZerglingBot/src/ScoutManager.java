...
public class ScoutManager {
	...
	private Unit currentOverloadScoutUnit;
	...
	{ 
		...
		{
			if(currentOverloadScoutUnit == null || currentOverloadScoutUnit.exists() == false || currentOverloadScoutUnit.getHitPoints() <= 0)
			{
				currentOverloadScoutUnit = null;
				currentOverloadScoutStatus = ScoutStatus.NoScout.ordinal();
					
				for(Unit unit : MyBotModule.Broodwar.self().getUnits())
				{
					if(unit.getType() == UnitType.Zerg_Overlord){
						currentOverloadScoutUnit = unit;
						break;
					}
				}
			}
	...
	/// 정찰 유닛을 이동시킵니다
	// 상대방 MainBaseLocation 위치를 모르는 상황이면, 
	// 일꾼은 StartLocation 들에 대해 아군의 MainBaseLocation에서 먼 곳부터 순서대로 정찰, 오버로드는 가까운 곳부터 정찰
	public void moveScoutUnit()
	{
		...
		if(enemyBaseLocation == null){
			if (currentScoutTargetBaseLocation == null || currentScoutUnit.getDistance(currentScoutTargetBaseLocation.getPosition()) < 5 * Config.TILE_SIZE) 
			{
				...
				if(closestBaseLocation != null) {
					commandUtil.move(currentOverloadScoutUnit, closestBaseLocation.getPosition());
				}
			}			
		}
		else
		{
			if (currentOverloadScoutUnit != null) {
				...
				if (MyBotModule.Broodwar.isExplored(currentScoutTargetBaseLocation.getTilePosition()) == false) {
					...
					commandUtil.move(currentOverloadScoutUnit, currentScoutTargetPosition);
				}
				else {
					currentOverloadScoutStatus = ScoutStatus.NoScout.ordinal();
		...
		if (enemyBaseLocation == null)
		{
			if (currentScoutTargetBaseLocation == null || currentScoutUnit.getDistance(currentScoutTargetBaseLocation.getPosition()) < 5 * Config.TILE_SIZE) 
			{
				...
				double tempfarDistance = 1000000000;
				BaseLocation farthestBaseLocation = null;
				for (BaseLocation startLocation : BWTA.getStartLocations())
				{
					// if we haven't explored it yet (방문했었던 곳은 다시 가볼 필요 없음)
					if (MyBotModule.Broodwar.isExplored(startLocation.getTilePosition()) == false)
					{
						// GroundDistance 를 기준으로 가장 먼 곳으로 선정
						tempfarDistance = (double)(InformationManager.Instance().getMainBaseLocation(MyBotModule.Broodwar.self()).getGroundDistance(startLocation) + 150);

						if (tempfarDistance > 0 && farthestDistance < tempfarDistance) {
							farthestBaseLocation = startLocation;
							farthestDistance = tempfarDistance;
						}
					}
				}

				if (farthestBaseLocation != null) {
					// assign a scout to go scout it
					commandUtil.move(currentScoutUnit, farthestBaseLocation.getPosition());
					currentScoutTargetBaseLocation = farthestBaseLocation;
				}
			}
		}
